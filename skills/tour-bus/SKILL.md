...
3. **Write the tour.** Roughly 3-8 sentences total, broken into short paragraphs: never more than 2-3 sentences per paragraph, so one to three paragraphs in all. It must be:
   ...
   - **ASCII-safe, with one exception.** Standard keyboard characters only: no em-dashes (restructure the sentence instead), no smart quotes, no decorative unicode of any kind. The one exception is mathematical symbols: when the subject is mathematical and a symbol is the natural notation (≥, ≠, ±, ×, √, ∞, and the like), use the unicode symbol rather than an ASCII stand-in. That is the only unicode allowed. Everywhere else, spell things out: an arrow used as a prose connector becomes "leads to", and ">=" in ordinary prose becomes "at least". This is the most easily broken rule and em-dashes especially will try to sneak in, so re-read the draft before delivering and rewrite any sentence that leans on one.
